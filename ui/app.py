...
.hero { background: linear-gradient(135deg, var(--bg-secondary) 0%, var(--bg-tertiary) 100%); border: 1px solid var(--border); border-left: 3px solid var(--accent); padding: 2.5rem 3rem; margin-bottom: 2rem; position: relative; overflow: hidden; }
.hero::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: radial-gradient(ellipse at top left, var(--accent-dim) 0%, transparent 60%); pointer-events: none; }
...
.metric-card { background: var(--bg-secondary); border: 1px solid var(--border); padding: 1.5rem; position: relative; border-radius: 6px; transition: all 0.2s; margin-bottom: 1rem; }
...
.code-viewer { background: var(--bg-primary); border: 1px solid var(--border); border-radius: 6px; font-family: 'JetBrains Mono', monospace; font-size: 0.85rem; line-height: 1.6; overflow-x: auto; }
.code-line { display: flex; min-height: 1.6rem; }
.code-line:hover { background: var(--bg-tertiary); }
.line-number { width: 50px; padding: 0.25rem 0.75rem; color: var(--text-muted); text-align: right; border-right: 1px solid var(--border); user-select: none; }
...
.sev-badge { display: inline-flex; align-items: center; gap: 0.25rem; font-size: 0.65rem; font-weight: 600; padding: 0.25rem 0.5rem; border-radius: 4px; text-transform: uppercase; letter-spacing: 0.05em; }
...
.terminal { background: #050505; border: 1px solid var(--border); border-radius: 6px; padding: 1rem; font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; max-height: 300px; overflow-y: auto; }
.log-info { color: var(--accent); } .log-warn { color: var(--medium); } .log-error { color: var(--critical); } .log-dim { color: var(--text-muted); }
.cost-card { background: var(--bg-secondary); border: 1px solid var(--border); border-top: 2px solid var(--accent); padding: 1.5rem; border-radius: 6px; }
.cost-value { font-family: 'JetBrains Mono', monospace; font-size: 2rem; font-weight: 700; color: var(--accent); }
.stTextInput input { background: var(--bg-secondary) !important; border: 1px solid var(--border) !important; color: var(--text-primary) !important; font-family: 'JetBrains Mono', monospace !important; border-radius: 6px !important; }
.stTextInput input:focus { border-color: var(--accent) !important; box-shadow: 0 0 0 2px var(--accent-dim) !important; }
.stButton button { background: var(--accent) !important; color: #000 !important; font-family: 'JetBrains Mono', monospace !important; font-weight: 700 !important; font-size: 0.8rem !important; border: none !important; border-radius: 6px !important; padding: 0.75rem 2rem !important; letter-spacing: 0.05em !important; text-transform: uppercase !important; }
.stButton button:hover { background: #00cc6a !important; }
.section-header { font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; color: var(--accent); text-transform: uppercase; letter-spacing: 0.15em; margin-bottom: 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid var(--border); }
...
with st.sidebar:
    ...
    st.markdown("""
    <div style="margin-top:1rem;padding-top:1rem;border-top:1px solid #1a1a1a;font-size:0.65rem;color:#333;line-height:1.8">
        <div style="color:#444;margin-bottom:0.25rem">// Stack</div>
        Gemini 2.0 Flash · Google ADK<br/>Pydantic v2 · GitHub REST API<br/><br/>
        <div style="color:#444;margin-bottom:0.25rem">// Status</div>
        84 tests passing<br/>CI green
    </div>
    """, unsafe_allow_html=True)
...
<div id="cmd-palette" style="display:none;position:fixed;top:20%;left:50%;transform:translateX(-50%);width:560px;max-width:90vw;background:#0f0f0f;border:1px solid #1e1e1e;border-radius:8px;z-index:9999;box-shadow:0 25px 80px rgba(0,0,0,0.9)">
    ...
    <div style="padding:0.5rem 1rem;border-top:1px solid #1e1e1e;font-size:0.65rem;color:#444">
...
if analyze_btn and repo_url:
    if not google_api_key:
        st.error("🔐 GOOGLE_API_KEY required")
        st.stop()
    if not github_token:
        st.error("🔐 GITHUB_TOKEN required")
        st.stop()
    ...
    try:
        ...
        with m1:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:#ff3232">{critical}</div><div class="metric-label">Critical</div></div>', unsafe_allow_html=True)
        with m2:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:#ff8c00">{high}</div><div class="metric-label">High</div></div>', unsafe_allow_html=True)
        with m3:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:#ffc800">{medium}</div><div class="metric-label">Medium</div></div>', unsafe_allow_html=True)
        with m4:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:#00ff88">{low}</div><div class="metric-label">Low</div></div>', unsafe_allow_html=True)
        ...
        if issues:
            ...
            with cc1:
                ...
                fig = px.pie(
                    ...
                    names=list(sd.keys()),
                    color_discrete_map={"CRITICAL": "#ff3232", "HIGH": "#ff8c00", "MEDIUM": "#ffc800", "LOW": "#00ff88"},
                    hole=0.6
                )
                fig.update_layout(
            ...
            for issue in issues[:5]:
                ...
                if fp and fp.endswith(".py"):
                    try:
                        ...
                        ires.append(r)
                        tnow += r["current_cost_usd"]
                        tfut += r["future_cost_usd"]
                    except Exception:
                        pass
            if ires:
                dc1, dc2, dc3 = st.columns(3)
                with dc1:
                    st.markdown(f'<div class="cost-card"><div style="font-size:0.7rem;color:#555;text-transform:uppercase;margin-bottom:0.5rem">Fix Cost Today</div><div class="cost-value">${tnow:,.0f}</div></div>', unsafe_allow_html=True)
                with dc2:
                    st.markdown(f'<div class="cost-card" style="border-top-color:#ff8c00"><div style="font-size:0.7rem;color:#555;text-transform:uppercase;margin-bottom:0.5rem">Cost Next Quarter</div><div class="cost-value" style="color:#ff8c00">${tfut:,.0f}</div></div>', unsafe_allow_html=True)
                with dc3:
                    st.markdown(f'<div class="cost-card" style="border-top-color:#ffc800"><div style="font-size:0.7rem;color:#555;text-transform:uppercase;margin-bottom:0.5rem">Save if Fixed Now</div><div class="cost-value" style="color:#ffc800">${tfut - tnow:,.0f}</div></div>', unsafe_allow_html=True)
                for r in ires:
        ...
        if fixes:
            ...
            for fix in fixes[:5]:
                with st.expander(f"🔧 {fix.get('fix_summary', 'Fix available')}"):
                    if fix.get("problem_summary"):
                        st.markdown(f"**Problem:** {fix['problem_summary']}")
                    if fix.get("steps"):
                        for step in fix["steps"]:
                            st.markdown(f"- {step}")
                    if fix.get("testing_tip"):
                        st.markdown(f"**Test:** {fix['testing_tip']}")
        ...
        if mode == "AutoPilot (Dry Run)":
            ...
            try:
                ...
                with a1:
                    st.markdown(f'<div class="metric-card"><div class="metric-value">{apr.get("files_analyzed", 0)}</div><div class="metric-label">Files Scanned</div></div>', unsafe_allow_html=True)
                with a2:
                    st.markdown(f'<div class="metric-card"><div class="metric-value">{apr.get("issues_found", 0)}</div><div class="metric-label">Issues Found</div></div>', unsafe_allow_html=True)
                with a3:
                    st.markdown(f'<div class="metric-card"><div class="metric-value">{len(apr.get("prs_created", []))}</div><div class="metric-label">PRs Simulated</div></div>', unsafe_allow_html=True)
                ...
                if ss:
                    ...
                    """, unsafe_allow_html=True)
                st.markdown(f'<div class="terminal"><span class="log-info">{ap.generate_report([apr])}</span></div>', unsafe_allow_html=True)
            except Exception as e:
